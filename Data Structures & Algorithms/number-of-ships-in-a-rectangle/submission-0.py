...
class Solution:
    def countShips(self, sea: 'Sea', topRight: 'Point', bottomLeft: 'Point') -> int:
        # if hasShips == 0; return 
        if not sea.hasShips(topRight, bottomLeft):
            return 0

        # if it is someway inverted wrongly
        if (bottomLeft.x > topRight.x) or (bottomLeft.y > topRight.y):
            return 0

        # if topRight == bottomLeft: return 1 -- basically found 1
        ...
        #recursively return topleft, topright, bottomleft, bottomright
        #remember it is topright, bottomleft
